...
void 
compute_on_device (float *gpu_result, float *matrix_c,\
...
main (int argc, char **argv)
{
    ...
    /* Convolve the matrix along the rows and columns. 
...
void 
print_kernel (float *kernel, int half_width)
{
    int i, j = 0;
...
/* Print matrix. */
void 
print_matrix (float *matrix, int num_cols, int num_rows)
{
    for (int i = 0; i < num_rows; i++) {
        for (int j = 0; j < num_cols; j++){
            float element = matrix[i * num_cols + j];
            printf ("%0.2f ", element);
        }
        printf ("\n");
    }
