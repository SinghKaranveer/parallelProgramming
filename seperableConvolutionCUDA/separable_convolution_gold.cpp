...
float *
create_kernel (float coeff, int half_width)
{
...
void 
convolve_rows (float *result, float *input, float *kernel,\
              int num_cols, int num_rows, int half_width)
{
    ...
    for (y = 0; y < num_rows; y++) {
        for (x = 0; x < num_cols; x++) {
            ...
            /* Obtain relative position of the starting element from 
             * element being convolved.
             * */
            i1 = j1 - x; 
...
void 
convolve_columns (float *result, float *input, float *kernel,\
                  int num_cols, int num_rows, int half_width)
{
    int i, i1;
    int j, j1, j2;
    ...
    for (y = 0; y < num_rows; y++) {
        for(x = 0; x < num_cols; x++) {
            ...
            /* Obtain relative position of the starting element 
             * from the element being convolved.
            */
            i1 = j1 - y; 
...
/* Perform separable convolution on the CPU. */
void 
compute_gold (float *matrix_a, float *matrix_b, float *kernel,\
              int num_cols, int num_rows, int half_width)
{
    /* Convolve over rows. 
     * matrix_a is the input matrix and the convolved matrix is stored in matrix_b.
    */	 
    printf ("Convolving over rows\n");
    convolve_rows (matrix_b, matrix_a, kernel, num_cols, num_rows, half_width);
	  
    /* Convolve over columns. 
     * matrix_b is the input matrix and the convolved matrix is stored in matrix_a.
     */
